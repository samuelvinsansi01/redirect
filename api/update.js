export default async function handler(req, res) {
  ...
  // Extrai os node-ids dos links do Figma recebidos
  ...
  // Extrai os IDs atuais da URL armazenada
  ...
  } catch { return res.status(500).json({ error: 'Não foi possível ler a URL atual' }); }

  // Usa o novo valor se fornecido, senão mantém o atual
  ...
  const updateRes = await fetch(`https://api.tinyurl.com/update/${alias}`, {
    method: 'PATCH',
    headers: {
